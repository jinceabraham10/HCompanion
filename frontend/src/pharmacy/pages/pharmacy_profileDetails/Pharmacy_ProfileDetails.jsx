...
import { pharmacy_getProfileDetailsService, pharmacy_updateProfileDetailsService } from '../../services/pharmacyProfileService';


function Pharmacy_ProfileDetails() {
  ...
  const formik=useFormik({
    ...
    onSubmit: async (values,actions)=>{
      ...
      const updated=await pharmacy_updateProfileDetailsService(values)
      if(updated){
        Swal.fire({
          icon:"success",
 ...
 const onLoad=async ()=>{
  const tempProfileDetails=await pharmacy_getProfileDetailsService()
  formik.setFieldValue('pharmacyName',tempProfileDetails.pharmacyName)
      ...
      <div className='w-full h-full flex flex-col items-center'>
        <form className='w-[50%] h-full flex flex-col gap-6 pt-10 '>
          ...
          <div className='w-full h-[15vh] flex gap-4 justify-center'>
           ...
           <FloatingLabel variant='filled' label='First Name' name='firstName' value={formik.values.pharmacyName} onChange={formik.handleChange}/>
           <FloatingLabel variant='filled' label='Last Name' name='lastName' value={formik.values.ownerName} onChange={formik.handleChange}/>
...
export default Pharmacy_ProfileDetails
