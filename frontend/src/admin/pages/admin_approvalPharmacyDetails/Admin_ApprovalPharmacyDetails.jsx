...
import { admin_approval_approvePharmacyService, admin_approval_getPharmacyDetailsService } from '../../services/admin_approvalServices';



function Admin_ApprovalPharmacyDetails() {


  const [userparams]=useSearchParams()
 const pharmacyId=userparams.get('pharmacyId')
...
const handleApprove=async (e)=>{
    // e.preventDefault();
    const approved=await admin_approval_approvePharmacyService({pharmacyId})
    if(approved){
   ...
   const onLoad=async ()=>{
    const tempDetails=await admin_approval_getPharmacyDetailsService({pharmacyId})
    if(tempDetails){
        formik.setFieldValue("pharmacyName",tempDetails.pharmacyName)
...
export default Admin_ApprovalPharmacyDetails