...
exports.paymentMedicineRequestVerification=async (req,res,next)=>{
    try {

        const { razorpay_order_id,razorpay_payment_id,razorpay_signature,pharmacyId,order}=req.body
        ...
        if(!paymentVerificationStatus)
            return res.status(400).json({message:"payment verification failed",paymentNotVerified:true})
        // const payer=await User({_id:req.user.userId})
        const reciever=await Pharmacy.findOne({_id:pharmacyId})
        // await console.log(`doctor ${JSON.stringify(Doctor)}`)
        const newPayment=new Payment({payerId:req.user.userId,recieverId:reciever.userId,paymentCategory:"bookingMedicine",paymentDetails:order})
        const savedPayment=await newPayment.save()
        if(savedPayment){
            req.user.paymentId=savedPayment._id
            return next()
